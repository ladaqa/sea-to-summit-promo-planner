...
- Select products (with Stock All *and* Stock FG/SDC1 shown per SKU) — or paste many article codes at once (any text; only real catalog codes are picked, unknown/no-price codes are reported), apply a Discount Tier, and stack up to 5 promo steps: On Top %, cumulative %, **One Price**, Cash Voucher, GWP (manual cost, or picked from a real Sea to Summit product's actual cost), **Bundle Deal** (search/pick up to 10 real catalog products into one set — or pull straight from your Step 1 selection — priced as a flat ฿ or a % off), and Redeem.
